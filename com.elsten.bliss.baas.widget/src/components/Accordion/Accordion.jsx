...
class Accordion extends React.Component {
  ...
  render() {
    const style = {overflow: 'hidden',transition: `height ${this.props.transitionDuration}ms ${this.props.transitionTimingFunction}`}
    const nodes = React.Children.map(this.props.children, (child, index) => ( child.props["header"] &&
      <div key={index} ref={div => { this.nodes[index] = { ref: div } }} className={this.state.index === index ? this.props.openClassName : ''}>
        <div onClick={() => this.toggle(index, true)}>{this.g(child)}</div>
